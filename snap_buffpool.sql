...
dec(((float(POOL_DATA_P_READS) + float(POOL_INDEX_P_READS) + float(POOL_TEMP_DATA_P_READS) + float(POOL_TEMP_INDEX_P_READS))/ float(COMMIT_SQL_STMTS)),9,2) as avg_phys_read_per_txn,
dec(((float(POOL_DATA_L_READS) + float(POOL_INDEX_L_READS) + float(POOL_TEMP_DATA_L_READS) + float(POOL_TEMP_INDEX_L_READS))/ float(COMMIT_SQL_STMTS)),9,2) as avg_logical_read_per_txn,
